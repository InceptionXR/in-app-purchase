...
module.exports.getPurchaseData = function (purchase, options) {
    ...
    if (purchase.receipt[REC_KEYS.IN_APP]) {
        ...
        if (lri && Array.isArray(lri)) {
            // adding first items from 'latest_receipt_info' due they have some extra field (eg - 'is_upgraded') comparing to 'in_app'
            list = list.concat(lri);
        }
        // taking rest items
        var inAppItems = purchase.receipt[REC_KEYS.IN_APP].filter(
            (ip) =>
                !list.some((lp) => lp.PURCHASE_DATE_MS === ip.PURCHASE_DATE_MS)
        );
        list = list.concat(inAppItems);
        ...
        */
        ...
        for (var i = 0, len = list.length; i < len; i++) {
            ...
            if (
                item[REC_KEYS.CANCELLATION_DATE] &&
                item[REC_KEYS.CANCELLATION_DATE].length &&
                item[REC_KEYS.CANCELLATION_REASON] === "0" &&
                bool(item[REC_KEYS.IS_UPGRADED])
            ) {
                continue;
            }
